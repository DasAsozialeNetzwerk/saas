...
onMessage server msg
   ...
   | "Amazon" `isIn` mMsg msg = hilightSend "https://stallman.org/amazon.html"
   | "Skype" `isIn` mMsg msg = hilightSend "https://stallman.org/skype.html"
   | "Google" `isIn` mMsg msg = hilightSend "https://stallman.org/google.html"
   | "Apple" `isIn` mMsg msg = hilightSend "https://stallman.org/apple.html"
   | "Facebook" `isIn` mMsg msg = hilightSend "https://stallman.org/facebook.html"
   | "Uber" `isIn` mMsg msg = hilightSend "https://stallman.org/uber.html"
   | "Netflix" `isIn` mMsg msg = hilightSend "https://stallman.org/netflix.html"
   | "Harry Potter" `isIn` mMsg msg = hilightSend "https://stallman.org/harry-potter.html"
   | " Linux" `isIn` mMsg msg = do -- TODO: schoenere Loesung finden
   ...
   where chan = fromJust $ mChan msg
         ...
         hilightSend msg = sendMsg server chan $ nick <> ": " <> msg
         isIn :: B.ByteString -> B.ByteString -> Bool
         s `isIn` msg = s `B.isInfixOf` msg
